...
export class CrawlerSitemapService {
   ...
   * @return {boolean}
   */
  async crawlerSitemapOfSite(data): Promise<void> {
    const { site, configServer } = data;
    let sitemaps = [];
    ...
    try {
      ...
      // Check dictionary sitemaps if not isset in robots.txt
        ...
        for (let i = 0; i < DICTIONARY_SITEMAP.length; i++) {
          const sitemap = `${site.url}/${DICTIONARY_SITEMAP[i]}`
          if (await this.checkTypeSitemap(sitemap, site, STATUS_SITE_MAP.ACTIVE)){
            statusSite = true;
          }
        }
      }
    ...
    // await this.gatewayRepository.updateStatusConfig(configServer, STATUS_CONFIG.ACTIVE)
    await this.crawlerRepository.updateSiteWithSitemap(site.url, statusSite)
    this.httpService.axiosRef.post(process.env.WEBHOOK_SITEMAP);
  }
   ...
   * @return {void}
   */
  async checkTypeSitemap(url: string, site: Site, statusSitemap: number): Promise<boolean> {
    try {
      const sitemap = await this.getResponseAxios(url)
   ...
   */
  async getResponseAxios(url: string): Promise<any> {
    try {
