...
class Product extends FrontendController{
    ...
    public function index($id = 0, $page = 1){
        ...
        $this->data['object'] = $this->AutoloadModel->_get_where([
            'select' => 'tb1.id,tb1.price,tb1.sub_album, tb1.price_promotion,tb1.catalogueid, tb1.viewed, tb1.album, tb1.image, tb2.title, tb2.canonical,tb2.sub_album_title, tb2.meta_title, tb2.sub_title,tb2.video,tb2.video_2, tb2.sub_content, tb1.productid, tb2.meta_description,  tb2.description, tb2.content, tb1.bar_code, tb1.model, tb1.brandid,tb1.time, tb1.articleid, tb1.icon, tb1.rate,  tb1.author, tb1.author_image, tb3.status as accept',
            ...
            'join' => [
                ...
                [
                    'lesson_relationship as tb3','tb1.id = tb3.productid AND tb3.memberid = \''.$memberid.'\' ','left'
                ]
            ],
        ]);
        ...
        $this->set_cookie($id, $this->data['object']);

        $this->data['object_next'] = $this->AutoloadModel->_get_where([
    ...
    private function connect_post($object = []){
        ...
        if(isset($id) && is_array($id) && count($id) && $id[0] != ''){
            $listPost = $this->AutoloadModel->_get_where([
                'select' => 'tb1.id, tb2.title, tb2.description, tb2.canonical, tb2.content',
    ...
    private function random_product($product_general = [], $id = 0){
        ...
        $product = $this->AutoloadModel->_get_where([
            'select' => 'tb1.id, tb2.title, tb1.price, tb1.price_promotion, tb2.canonical, tb1.image, tb3.title as brand',
            'table' => 'product as tb1',
            'join' => [
    ...
    private function product_general($module_extract = [], $id = []){
        $product_general = $this->AutoloadModel->_get_where([
            'select' => 'tb1.catalogueid, tb3.id, tb3.price, tb3.price_promotion, tb3.image, tb2.module, tb3.rate, tb3.album,       tb2.title,tb2.description, tb2.content, tb2.canonical,  tb2.meta_title, tb2.meta_description, tb3.viewed',
