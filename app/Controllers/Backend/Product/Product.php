...
class Product extends BaseController{
	...
	public function index($page = 1){
		...
		$member_list = $this->AutoloadModel->_get_where([
			'select' => 'tb1.id, tb1.email',
			'table' => 'member as tb1',
			'where' => [
				'tb1.deleted_at' => 0,
				'tb1.publish' => 1
			],
			'order_by' => 'tb1.email asc'
		],true);
		...
		if(isset($member_list) && is_array($member_list) && count($member_list)){
			foreach ($member_list as $value) {
			    $this->data['member_list'][$value['id']] = $value['email'];
			}
		}
		...
		if($config['total_rows'] > 0){
			...
			$this->data['productList'] = $this->AutoloadModel->_get_where([
				'select' => 'tb1.id, tb1.catalogueid as cat_id, tb1.price,tb1.order, tb1.price_promotion, tb1.bar_code, tb1.model, tb1.trending, tb1.featured,  tb1.brandid, tb1.album,  tb2.catalogueid, tb1.publish, tb3.title as product_title, tb1.catalogue, tb2.objectid, tb3.content, tb3.sub_title, tb3.sub_content, tb3.canonical, tb1.image, tb3.meta_title, tb3.meta_description, tb3.made_in, tb4.title as cat_title ,'.((isset($languageDetact['select'])) ? $languageDetact['select'] : ''),
				'table' => $this->data['module'].' as tb1',
				'where' => $where,
				'where_in' => $catalogue['where_in'],
				'where_in_field' => $catalogue['where_in_field'],
	...
	public function create(){
		...
		if(!isset($this->data['check_code']) && !is_array($this->data['check_code'])){
			$session->setFlashdata('message-danger', 'Bạn chưa tạo phần cấu hình chung cho mã Khóa học!');
					...
					$wholesale = $this->request->getPost('wholesale');
			 		...
			 		if($resultid > 0){
						...
						$this->nestedsetbie->Action();
			 			$session->setFlashdata('message-success', 'Tạo Khóa học Thành Công! Hãy tạo danh mục tiếp theo.');
	...
	public function update($id = 0){
		...
		if($this->data[$this->data['module']] == false){
			$session->setFlashdata('message-danger', 'Khóa học không tồn tại!');
 			return redirect()->to(BASE_URL.'backend/product/product/index');
		}
		$this->data['get_canonical'] = $this->get_canonical();
				...
				$wholesale = $this->request->getPost('wholesale');
		 		...
		 		if($flag > 0){
					...
					$this->nestedsetbie->Action();
		 			...
		 			$session->setFlashdata('message-success', 'Cập Nhật Khóa học Thành Công!');
 					return redirect()->to(BASE_URL.'backend/product/product/index');
		 		}
	...
	private function storeLanguage($objectid = 0){
		...
		$store = [
			...
			'video_2' => $this->request->getPost('video_2'),
	...
	private function get_data_module($id = 0){
		$session = session();
		$flag = $this->AutoloadModel->_get_where([
			'select' => 'tb1.id, tb1.catalogue,tb1.time_end,tb1.sub_album,tb2.sub_album_title,tb2.video, tb2.video_2, tb1.bar_code, tb1.brandid, tb1.time,tb1.image,tb1.author, tb1.author_image, tb1.catalogueid, tb1.model, tb1.price_promotion, tb1.price, tb1.productid, tb1.id, tb1.id, tb1.id, tb2.title, tb2.objectid, tb2.sub_title, tb2.sub_content, tb2.description, tb2.canonical,  tb2.content, tb2.meta_title, tb2.meta_description, tb1.album, tb1.publish, tb1.productid_original, tb1.articleid, tb1.icon',
			'table' => $this->data['module'].' as tb1',
			'join' =>  [
					[
	...
	private function validation(){
		...
		$errorValidate = [
			'title' => [
				'required' => 'Bạn phải nhập tên Khóa học!'
			],
			'productid' => [
				'required' => 'Bạn phải nhập mã Khóa học!',
				'check_id' => 'Mã Khóa học đã tồn tại, vui lòng chọn mã Khóa học khác!',
			],
			'price' => [
				'required' => 'Bạn phải nhập giá Khóa học!'
			],
			'canonical' => [
			...
			'catalogueid' => [
				'is_natural_no_zero' => 'Bạn Phải chọn danh mục cha cho Khóa học!',
			],
		];
