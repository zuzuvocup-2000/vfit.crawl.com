...
class Catalogue extends BaseController{
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($resultid > 0){
					...
					$this->nestedsetbie->Action();
		 			$session->setFlashdata('message-success', 'Tạo Nhóm Khóa học Thành Công! Hãy tạo danh mục tiếp theo.');
	...
	public function update($id = 0){
		...
		if($this->data[$this->data['module']] == false){
			$session->setFlashdata('message-danger', 'Nhóm Khóa học không tồn tại!');
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($flag > 0){
					...
					$this->nestedsetbie->Action();
		 			...
		 			$session->setFlashdata('message-success', 'Cập Nhật Nhóm Khóa học Thành Công!');
 					return redirect()->to(BASE_URL.'backend/product/catalogue/index');
		 		}
