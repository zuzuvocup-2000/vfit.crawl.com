...
class Catalogue extends BaseController{
	...
	public function __construct(){
		$this->data = [];		
		$this->data['module'] = 'author_catalogue';
	}

	public function index($page = 1){
		$session = session();
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/author/catalogue/index'
		]);
		...
		if($config['total_rows'] > 0){
			$config = pagination_config_bt(['url' => 'backend/author/catalogue/index','perpage' => $perpage], $config);
			...
			$this->data['authorCatalogueList'] = $this->AutoloadModel->_get_where([
				'select' => 'id, title, userid_created, userid_updated, publish',
				'table' => $this->data['module'],
				'where' => $where,
				'keyword' => $keyword,
		...
		$this->data['template'] = 'backend/author/catalogue/index';
	...
	public function create(){
		if($this->request->getMethod() == 'post'){
			...
			$errorValidate = [
				'title' => [
					'required' => 'Bạn bắt buộc phải nhập vào ô Tiêu đề nhóm Tác giả!',
				],
			];
			if ($this->validate($validate, $errorValidate)){
		 		...
		 		if($insertid > 0){
		 			...
		 			return redirect()->to(BASE_URL.'backend/author/catalogue/index');
		 		}
	        }else{
		...
		$this->data['template'] = 'backend/author/catalogue/store';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function update($id = 0){
		...
		if(!isset($this->data[$this->data['module']]) || is_array($this->data[$this->data['module']]) == false || count($this->data[$this->data['module']]) == 0){
			$session->setFlashdata('message-danger', 'Nhóm Tác giả không tồn tại');
 			return redirect()->to(BASE_URL.'backend/author/catalogue/index');
		}
		if($this->request->getMethod() == 'post'){
			$validate = [
	...
	public function delete($id = 0){
		...
		$this->data['template'] = 'backend/author/catalogue/delete';
		return view('backend/dashboard/layout/home', $this->data);
	}
