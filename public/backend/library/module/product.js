...
$(document).on('click','.open-modal-code',function(){
	let _this = $(this);
	let productid = _this.parents('tr').attr('data-id')
	$('.btn-create-code').attr('data-productid', productid)
	render_code_lesson(productid)
});

$(document).on('click','.btn-create-code',function(){
	let _this = $(this);
	let id = $('.id-member-code').val()
	let code = $('.random-input').val()
	let productid = _this.attr('data-productid')
	if(id == '' || code == ''){
		toastr.error('Xin vui lòng điền đầy đủ thông tin các trường để tiếp tục');
	}else{
		$.post('ajax/code/add_code', {
			code: code,id :id,productid :productid},
		function(data){
			let response = JSON.parse(data);
			if(response.code == 10){
				$('.form-code-lesson')[0].reset();
				render_code_lesson(productid);
				toastr.success(response.message);
			}else{
				if(response.code != 10){
...
function render_code_lesson(productid = 0){
	if(productid != 0){
		$.post('ajax/code/render_lesson', {
			productid :productid},
		function(data){
			let response = JSON.parse(data);
			$('.list-code-user').html(response.html)
		});
	}
}
